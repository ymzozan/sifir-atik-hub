        ...
        {ranked.map((school, i) => {
          ...
          const leagueHead =
            rank === 1
              ...
              : null;
          // Yükselme: alt liglerin zirvesi · Düşme: üst liglerin dibi
          const promote = rank === third + 1 || rank === third * 2 + 1;
          const relegate =
            (rank === third || rank === third * 2) && rank !== ranked.length;
          return (
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {promote && (
                      <span className="inline-flex shrink-0 items-center gap-0.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-extrabold text-emerald-700">
                        <TrendingUp className="h-3 w-3" /> Yükselme
                      </span>
                    )}
                    {relegate && (
                      <span className="inline-flex shrink-0 items-center gap-0.5 rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-extrabold text-rose-600">
                        <TrendingUp className="h-3 w-3 rotate-180" /> Düşme
                      </span>
                    )}
                  </div>
...
const QUIZ_QUESTIONS: QuizQ[] = [
  ...
  {
    q: "Alüminyum kutu geri dönüşümü, sıfırdan üretime göre yaklaşık ne kadar enerji tasarrufu sağlar?",
    options: ["%5", "%25", "%50", "%95"],
    correct: 3,
    fact: "Alüminyumun geri dönüşümü, sıfırdan üretime göre yaklaşık %95 enerji tasarrufu sağlar.",
  },
  {
    q: "Sıfır atık hiyerarşisinde ilk ve en etkili adım hangisidir?",
    options: ["Geri dönüştür", "Azalt (önle)", "Yeniden kullan", "Geri kazan"],
    correct: 1,
    fact: "En etkili adım, atığı en baştan azaltmak — yani önlemektir.",
  },
  {
    q: "Organik atıklardan elde edilen doğal gübreye ne ad verilir?",
    options: ["Kompost", "Silaj", "Biyogaz", "Humus asidi"],
    correct: 0,
    fact: "Organik atıklar çürütülerek 'kompost' adı verilen doğal gübreye dönüşür.",
  },
  {
    q: "Denizlerdeki kirliliğin büyük bölümünü hangi atık oluşturur?",
    options: ["Cam", "Metal", "Plastik", "Kâğıt"],
    correct: 2,
    fact: "Deniz kirliliğinin büyük kısmı plastik atıklardan kaynaklanır.",
  },
  {
    q: "Kullanılmış kızartma yağı lavaboya dökülürse ne olur?",
    options: ["Boruları temizler", "Su kaynaklarını kirletir", "Hiçbir şey olmaz", "Gübreye dönüşür"],
    correct: 1,
    fact: "1 litre atık yağ, binlerce litre suyu kirletebilir; ayrı toplanmalıdır.",
  },
];
...
function QuizScreen({
  ...
  const [quizSet, setQuizSet] = useState(() =>
    pickRandom(QUIZ_QUESTIONS, QUIZ_LEN)
  );
  ...
  const q = quizSet[idx];
  const isLast = idx === quizSet.length - 1;
  ...
  const restart = () => {
    setQuizSet(pickRandom(QUIZ_QUESTIONS, QUIZ_LEN));
    setIdx(0);
    setPicked(null);
    setScore(0);
    setEarned(0);
      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <div className="mb-5 h-2 w-full overflow-hidden rounded-full bg-slate-100">
          <div
            ...
            style={{
              width: `${((idx + (picked !== null ? 1 : 0)) / quizSet.length) * 100}%`,
            }}
          />
        </div>
...
function TournamentScreen({ schools }: { schools: School[] }) {
  ...
  const [counts, setCounts] = useState<number[]>(() =>
    SPORTS_EVENTS.map((e) => e.teams * 18 + 60)
  );
  // Canlı katılımcı sayacı — başka okullar da katılıyormuş gibi artar
  useEffect(() => {
    const id = setInterval(() => {
      setCounts((c) => c.map((n) => n + Math.floor(Math.random() * 3)));
    }, 2500);
            ...
            return (
              ...
              >
                ...
                <p className="mt-1.5 inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600">
                  ...
                  {formatNumber(counts[i])} öğrenci katıldı
                </p>
                <button
                  onClick={() => {
                    setJoined((p) =>
